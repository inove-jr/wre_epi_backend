            ...
            <div class="pedido" v-for="pedido in pedidos" :key="pedido.id">
                ...
                <div class="pedido-details">
                    ...
                    <div>
                        ...
                        <select v-model="pedido.order_status" id="status">
                ...
                <div class="value">
                    <strong> Valor:</strong> R$ {{ pedido.valor_total.toLocaleString("pt-BR", {
                        minimumFractionDigits: 2
                    }) }}
                </div>
                <div class="botoes">
                    <button class="datalhesBnt" @click="showDetails(pedidos.indexOf(pedido))">Detalhes</button>
                    <button class="save-button" @click="saveStatus(pedidos.indexOf(pedido), pedido.order_status, pedido.order_id)">Salvar</button>
                    <!-- <button class="cancelarBnt">Cancelar</button> -->
                </div>

                <section class="overlay_blur" v-if="this.popControl[pedidos.indexOf(pedido)]"
                    @click="closeDetails(pedidos.indexOf(pedido))">
                </section>
...
export default {
    ...
    methods: {
        ...
        async saveStatus(index, newStatus,orderId) {
            const url = baseApiUrl+'/order-update'
            const data={
                orderId: orderId,
                newStatus: newStatus
            }
            ...
            try {
                ...
                alert("Novo Status Salvo com Sucesso!")
            } catch (error) {
                console.error(error)
                alert("Algo Deu Erro Alvo Salvar Novo Status, Tente Novamente.")
            }   
        }   
    },
    computed: {
